...
if helix:
    ...
    sourcetoaxis_dist = jvm['Configuration'].getGlobalConfiguration().getGeometry().getSourceToAxisDistance()
    angular_inc = traj.getAverageAngularIncrement()
    offset_u = jvm.enumval_from_int('Projection$CameraAxisDirection', traj.getDetectorOffsetU())
    offset_v = jvm.enumval_from_int('Projection$CameraAxisDirection', traj.getDetectorOffsetV())
...
grid = jvm['OpenCLGrid3D'](jvm['NumericalSheppLogan3D'](traj.getReconDimensionX(), traj.getReconDimensionY(), traj.getReconDimensionZ()).getNumericalSheppLoganPhantom())


grid.setOrigin(JArray(JDouble)([-traj.getOriginInPixelsX(), -traj.getOriginInPixelsY(), -traj.getOriginInPixelsZ()]))
grid.setSpacing(JArray(JDouble)([traj.getVoxelSpacingX(), traj.getVoxelSpacingY(), traj.getVoxelSpacingZ()]))
java.lang.System.out.println("GT: %f" % grid.getGridOperator().normL1(grid))
try:
    sino = jvm['OpenCLGrid3D'](jvm['Grid3D'](traj.getDetectorWidth(), traj.getDetectorHeight(), traj.getProjectionStackSize()))
    ...
    java.lang.System.out.println("RMSE: "+ str(foo.getGridOperator().rmse(foo, grid)))
    foo.show()
